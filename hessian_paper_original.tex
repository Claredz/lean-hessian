...
\usepackage{iftex}
\ifPDFTeX
  \PackageError{hessian-paper}{请使用 XeLaTeX 编译本文档}{推荐命令：xelatex -shell-escape hessian_paper_original.tex}
\fi
\usepackage{fontspec}
\usepackage{xeCJK}
...
\IfFontExistsTF{JuliaMono}{
  ...
  % TODO: 未安装 JuliaMono 时使用 DejaVu Sans Mono 回退；建议安装 JuliaMono 以获得最佳 Lean Unicode 渲染。
  \setmonofont{DejaVu Sans Mono}[Scale=MatchLowercase]
}

% 中文字体沿用 ctexart 在当前系统上的默认 Windows 字体配置。
% TODO: 当前环境未检测到 Noto Serif CJK SC / Noto Sans Mono CJK SC；如需统一跨平台字体，可安装后再显式启用。
...
\subsection{根模块 \texttt{Hessian.lean}}
\inputminted{lean4}{Hessian.lean}

\subsection{主形式化文件 \texttt{Hessian/Basic.lean}}
\inputminted{lean4}{Hessian/Basic.lean}
